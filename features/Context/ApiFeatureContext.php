...
class ApiFeatureContext implements Context
{
     ...
     */
    public function iSaveIdFromResponseHeaders()
    {
        $this->id = $this->getLastResponse()->getHeader('id')[0];
        file_put_contents(__DIR__ . '/../../var/id_behat', $this->id);
    }

    /**
     * @Then /I save uuid from response headers/
     */
    public function iSaveUuidFromResponseHeaders()
    {
        $uuid = $this->getLastResponse()->getHeader('uuid')[0];
        file_put_contents(__DIR__ . '/../../var/uuid_behat', $uuid);
    }

    /**
     * @When /^I request "(GET|PUT|POST|DELETE|PATCH) ([^"]*)" with id from response headers as last url part$/
     */
    public function iRequestWithIdFromHeaders($httpMethod, $resource)
    {
        $method = strtoupper($httpMethod);

        $resource .= ('/' . $this->getLastResponse()->getHeader('id')[0]);
    ...
    /**
     * @When /^I request "(GET|PUT|POST|DELETE|PATCH) ([^"]*)" with id that saved before as last url part$/
     */
    public function iRequestWithIdSavedBefore($httpMethod, $resource)
    {
        $method = strtoupper($httpMethod);

        $this->id = file_get_contents(__DIR__ . '/../../var/id_behat');
        $resource .= ('/' . $this->id);
    ...
    /**
     * @When /^I request "(GET|PUT|POST|DELETE|PATCH) ([^"]*)" with uuid that saved before as last url part$/
     */
    public function iRequestWithUuidSavedBefore($httpMethod, $resource)
    {
        $method = strtoupper($httpMethod);

        $this->id = file_get_contents(__DIR__ . '/../../var/uuid_behat');
        $resource .= ('/' . $this->id);
    ...
    public function theResponseShouldContainJson(PyStringNode $jsonString)
    {
        ...
        $actual = json_decode($this->getLastResponse()->getBody()->getContents(), true);
        if (null === $etalon) {
